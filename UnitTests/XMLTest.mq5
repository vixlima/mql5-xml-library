...
class PrivateAccessXML : public XML{
   public:
      int AccessParseNode(XMLNode *node, string text, int position = 0){
         return ParseNode(node, text, position);
      }
            
      int AccessParseAttribute(XMLNode *node, string text, int position){
         return ParseAttribute(node, text, position);
      }
      
      int AccessParseContent(XMLNode* node, string text, int position){
...
CUnitTest* ParseAttribute_Test(){
   ...
   XMLAttribute *attribute, *attributeSon;
