...
class XMLNode{
   ...
   public:
      ...
      void AddChild(XMLNode* child); // Add a child to the node
      XMLNode* DetachChildren(); // Detach all chidren (Be careful: you have to handle the children or you will loose memory)
      ...
      XMLNode* GetChild(string name); // Get a children by its name
      
      // Operations on brothers
      void AddLittleBrother(XMLNode* brother); // Add a brother to the node
      XMLNode* DetachLittleBrothers(); // Detach all little brothers from this node
...
void XMLNode::WithForbiddenLetters(string& entryString){
...
void XMLNode::WithoutForbiddenLetters(string& entryString){
   StringReplace(entryString, "<", "&#60"); // Replacement of <
   StringReplace(entryString, "=", "&#61"); // Replacement of =
   StringReplace(entryString, ">", "&#62"); // Replacement of >
}
...
string XMLNode::GenerateText(int depth = 0, bool brothers = true){
   string text = "";
   string shift = "";
...
void XMLNode::ReadAll(int depth=0){
...
void XMLNode::AddAttribute(string name,string text){
   this.WithoutForbiddenLetters(name);
   ...
   if(m_attributes!=NULL){
      ...
      m_attributes = new XMLAttribute(name, text);
   }
}
...
XMLAttribute* XMLNode::GetAttribute(string name){
   // Searching for the attribute
   for(XMLAttribute* attributeTemp = m_attributes; // Get the first attribute
...
XMLNode* XMLNode::DetachChildren(){
   XMLNode* temp = NULL;
...
void XMLNode::DeleteChild(){
   // Preservation of children and brothers 
   XMLNode* children = m_children.DetachChildren();
   XMLNode* brothers = m_children.DetachLittleBrothers();
...
XMLNode* XMLNode::GetChild(string name){
   for(XMLNode* node = m_children; // Get children
...
void XMLNode::AddLittleBrother(XMLNode *brother){
...
XMLNode* XMLNode::DetachLittleBrothers(){
   XMLNode* temp = NULL;
...
void XMLNode::DeleteLittleBrothers(){
   if(m_brothers!=NULL){
...
XMLAttribute* XMLNode::GetAttribute(){
   return m_attributes;
}
...
void XMLNode::SetComment(string comment){
